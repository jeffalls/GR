...
import alls.tech.gr.GR.packageName
...
class Location(recipient: String, sender: String) : CommandModule("Location", recipient, sender) {
    ...
    fun teleport(args: List<String>) {
        ...
        if (args.isEmpty()) {
            ...
            if (!status) {
                ...
                return GR.showToast(Toast.LENGTH_LONG, "Teleportation disabled")
            }

            return GR.showToast(Toast.LENGTH_LONG, "Please provide a location")
        }
                ...
                coroutineScope.launch {
                    ...
                    if (location != null) {
                        ...
                        if (apiLocation != null) {
                            ...
                            GR.showToast(Toast.LENGTH_LONG, "Location not found")
                        }
                    }
                }
                return
    ...
    fun save(args: List<String>) {
        if (args.isEmpty()) {
            GR.showToast(Toast.LENGTH_LONG, "Please provide a name for the location")
            return
        }
                ...
                if (coordinates.size == 2) {
                    try {
                        ...
                        if (existingLocation != null) {
                            ...
                            GR.showToast(Toast.LENGTH_LONG, "Successfully updated $name")
                        } else {
                            addLocation(name, lat, lon)
                            GR.showToast(Toast.LENGTH_LONG, "Successfully saved $name")
                        }
                    } catch (e: Exception) {
                        GR.showToast(Toast.LENGTH_LONG, "Invalid coordinates format")
                    }
                } else {
                    GR.showToast(Toast.LENGTH_LONG, "Invalid coordinates format")
                }
            } else {
                GR.showToast(Toast.LENGTH_LONG, "No location provided")
            }
        }
    }
    ...
    fun delete(args: List<String>) {
        if (args.isEmpty()) {
            return GR.showToast(Toast.LENGTH_LONG, "Please provide a location to delete")
        }
        ...
        coroutineScope.launch {
            ...
            deleteLocation(name)
            GR.showToast(Toast.LENGTH_LONG, "Location deleted")
        }
    }

    private suspend fun getLocation(name: String): Pair<Double, Double>? =
        withContext(Dispatchers.IO) {
            val locationDao = GR.database.teleportLocationDao()
    ...
    private suspend fun addLocation(name: String, latitude: Double, longitude: Double) =
        withContext(Dispatchers.IO) {
            val locationDao = GR.database.teleportLocationDao()
            val entity =
                alls.tech.gr.persistence.model.TeleportLocationEntity(
                    name = name,
                    latitude = latitude,
                    longitude = longitude
                )
    ...
    private suspend fun getLocationFromNominatimAsync(location: String): Pair<Double, Double>? =
        withContext(Dispatchers.IO) {
            ...
            return@withContext try {
                ...
                withContext(Dispatchers.Main) { GR.showToast(Toast.LENGTH_LONG, message) }
                Logger.apply {
                    e(message)
    ...
    private fun teleportToCoordinates(lat: Double, lon: Double, silent: Boolean = false) {
        ...
        GR.executeAsync {
            try {
                GR.httpClient.updateLocation(geohash)
            } catch (e: Exception) {
        ...
        if (!silent)
            GR.showToast(Toast.LENGTH_LONG, "Teleported to $lat, $lon")
    }
